...
namespace SBM.WEB.Repositories
{
    public class Repository : IRepository
    {
        ...
        public async Task<HttpResponseWrapper<T>> Get<T>(string url)
        {
            ...
            {
                var response = await UnserializeAnswer<Response<T>>(responseHttp, _jsonDefaultOptions);
        ...
        public async Task<HttpResponseWrapper<TResponse>> Post<T, TResponse>(string url, T model)
        {
            ...
            if (responseHttp.IsSuccessStatusCode)
            {
                var apiResponse = await UnserializeAnswer<Response<TResponse>>(responseHttp, _jsonDefaultOptions);
                return new HttpResponseWrapper<TResponse>(apiResponse, false, responseHttp);
            }

            return new HttpResponseWrapper<TResponse>(null, true, responseHttp);
        }
        ...
        public async Task<HttpResponseWrapper<TResponse>> Put<T, TResponse>(string url, T model)
        {
            ...
            if (responseHttp.IsSuccessStatusCode)
            {
                var response = await UnserializeAnswer<Response<TResponse>>(responseHttp, _jsonDefaultOptions);
